...
fn main() {
    ...
    primary::thread_local::install(
        AdHocTryDropStrategy(|error| println!("error from the main thread local: {error}"))
    );
...

    {
        ...
        let _guard = primary::thread_local::scope(AdHocTryDropStrategy(|error| println!("error from the first thread local scope: {error}")));
...

    {
        ...
        let _guard = primary::thread_local::scope(AdHocTryDropStrategy(|error| println!("error from the second thread local scope: {error}")));
...

    {
...

        {
            ...
            let error = primary::thread_local::ScopeGuard::try_new(UnreachableDropStrategy::safe()).unwrap_err();
            println!("the error comes in a form of a `LockedError`: debug={error:?}, display={error}")
        }
    }
}